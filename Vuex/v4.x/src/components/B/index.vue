<template>
    <div class='B'>
        <h2>{{ title }}</h2>
        <h3>计算结果：{{ store.state.ModuleB.BCalcResult }}</h3>
        <h3>放大百倍 通过 直接 去触发 缓存机制效果不好：{{ store.getters.BEnlarge }}</h3>
        <h3>放大百倍 通过 computed 去触发 缓存机制正常：{{ BEnlarge }}</h3>
        <h3>放大百倍 通过 computed 去触发 缓存机制正常：{{ BEnlargeActivity }}</h3>
...
import { ONBINCREASE, ONBDECREMENT, } from '@/store/Module/B/Mutation-types';
import { ONACTIONBINCREASE, ONACTIONBDECREMENT, } from '@/store/Module/B/Actions-types';
const store = useStore();
console.log(store);
const title = ref('B模块');
const SelectNumder = ref(1);
const CalcResult = ref(1);
const BEnlarge = computed(() => store.getters.BEnlarge);
const BEnlargeActivity = computed(() => store.getters.BEnlargeActivity(SelectNumder.value));
const onIncrease = () => {
    store.commit({ type: ONBINCREASE, Cardinality: CalcResult.value });
};
const onDecrement = () => {
    store.commit({ type: ONBDECREMENT, Cardinality: CalcResult.value });
};
const onSelectIncrease = () => {
    store.dispatch(ONACTIONBINCREASE, SelectNumder.value);
};
const onSelectDecrement = () => {
    store.dispatch({ type: ONACTIONBDECREMENT, Cardinality: SelectNumder.value });
};
const onReplaceState = () => {
    // 替换 Store 状态，一般用于重置状态，或者新增属性，对状态进行合并
    store.replaceState({ ...store.state, ModuleB: { BCalcResult: 0 }, time: new Date() });
};
