<template>
    <div class='A'>
        <h2>{{ title }}</h2>
        <h3>计算结果：{{ store.state.ModuleA.ACalcResult }}</h3>
        <h3>放大百倍 通过 直接 去触发 缓存机制效果不好：{{ store.getters.AEnlarge }}</h3>
        <h3>放大百倍 通过 computed 去触发 缓存机制正常：{{ AEnlarge }}</h3>
        <h3>放大百倍 通过 computed 去触发 缓存机制正常：{{ AEnlargeActivity }}</h3>
...
import { ONAINCREASE, ONADECREMENT, } from '@/store/Module/A/Mutation-types';
import { ONACTIONAINCREASE, ONACTIONADECREMENT, } from '@/store/Module/A/Actions-types';
const store = useStore();
const title = ref('A模块 -- 模块的局部状态');
const SelectNumder = ref(1);
const CalcResult = ref(1);
const AEnlarge = computed(() => store.getters.AEnlarge);
const AEnlargeActivity = computed(() => store.getters.AEnlargeActivity(SelectNumder.value));
const onIncrease = () => {
    store.commit({ type: ONAINCREASE, Cardinality: CalcResult.value });
};
const onDecrement = () => {
    store.commit({ type: ONADECREMENT, Cardinality: CalcResult.value });
};
const onSelectIncrease = () => {
    store.dispatch(ONACTIONAINCREASE, SelectNumder.value);
};
const onSelectDecrement = () => {
    store.dispatch({ type: ONACTIONADECREMENT, Cardinality: SelectNumder.value });
};
const onReplaceState = () => {
    // 替换 Store 状态，一般用于重置状态，或者新增属性，对状态进行合并
    store.replaceState({ ...store.state, ModuleA: { ACalcResult: 0 }, time: new Date() });
};
